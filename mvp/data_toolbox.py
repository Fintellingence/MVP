...
def loadFromDB_M1(symbol,db_filename='BRSharesMetaTrader_M1.db'):
    ...
    exist_err_msg = "ERROR : MetaTrader database file {} does not exists".format(full_db_path)
    if (not os.path.isfile(full_db_path)): raise IOError(exist_err_msg)
...
def fromM1_to_D1(df1M):
    ...
    newDF = pd.DataFrame(bar_list,columns=['Open','High','Low','Close','TickVol','Volume'],index=date_index)
...
def newTimeFrequencyDF(df1M,time_step=60):
    ...
    for DFindex in range(df1M.index.size):
        ...
        if (df1M.index[DFindex].minute % time_step == 0):
            ...
            nbars = nbars + 1
    newDF = pd.DataFrame(bar_list,columns=['Open','High','Low','Close','TickVol','Volume'],index=closeTime_index)
...
def tickBarsDF(df,barsize_threshold=1000):
    ...
    for DFindex in range(df.index.size):
        ...
        if accum_ticks > barsize_threshold:
            ...
            bar_list.append([bar_initialtime,bar_opn,bar_max,bar_min,bar_cls,accum_ticks,bar_vol])
            closeTime_index.append(bar_finaltime)
            accum_ticks = 0
            last_DFindex = DFindex+1
            bar_initialtime = bar_finaltime
            nbars = nbars + 1
    newDF = pd.DataFrame(bar_list,columns=['OpenTime','Open','High','Low','Close','TickVol','Volume'],index=closeTime_index)
...
def volumeBarsDF(df,barsize_threshold=1E6):
    ...
    for DFindex in range(df.index.size):
        ...
        if accum_vol > barsize_threshold:
            ...
            bar_opn = df.iloc[last_DFindex]['Open']
            bar_cls = df.iloc[DFindex]['Close']
            bar_list.append([bar_initialtime,bar_opn,bar_max,bar_min,bar_cls,accum_ticks,accum_vol])
            closeTime_index.append(bar_finaltime)
            accum_ticks = 0
            accum_vol = 0.
            last_DFindex = DFindex+1
...
def drawCandleStick(axis,data,color_up=DEFAULT_CUP,color_down=DEFAULT_CDOWN):
    ...
    if data['Close'] > data['Open']: color = color_up
    else                           : color = color_down
    # plot vertical line segment corresponding to maximun/minimum deals
    axis.plot([data['SeqNum'],data['SeqNum']],[data['Low'],data['High']],lw=1.5,color='black',solid_capstyle='round',zorder=2)
    x0 = data['SeqNum']-0.25
    y0 = data['Open']
    width=0.5
    height=data['Close']-data['Open']
    candleBody = mpl.patches.Rectangle((x0,y0),width,height,facecolor=color,edgecolor='black',lw=1,zorder=3)
...
def plotTimeCandles(df1M,start,stop,time_step=1):
    ...
    for timeIndex in plotDF.index:
        ...
        if (timeIndex.day > currentDay):
            ...
            ax[1].axvline(plotDF.loc[timeIndex]['SeqNum'],lw=2,ls='--',zorder=1,color='black')
            currentDay = timeIndex.day
    # Plot volume above the prices
    ax[0].vlines(plotDF['SeqNum'],np.zeros(plotDF.shape[0]),plotDF['Volume'],lw=4,zorder=2)
